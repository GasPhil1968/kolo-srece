...
self.addEventListener('install', function(e){
  e.waitUntil(caches.open(KES).then(function(c){ return c.addAll(JEZGRA); }).then(function(){ return self.skipWaiting(); }));
});
self.addEventListener('activate', function(e){
  e.waitUntil(caches.keys().then(function(kljucevi){
...
self.addEventListener('fetch', function(e){
  ...
  if(req.mode === 'navigate'){
    e.respondWith(fetch(req).then(function(odg){
      var kopija = odg.clone();
      caches.open(KES).then(function(c){ c.put(JEZGRA[1], kopija); });
      return odg;
    }).catch(function(){ return caches.match(JEZGRA[1]); }));
    return;
  }
  e.respondWith(caches.match(req).then(function(pogodak){
    if(pogodak) return pogodak;
    return fetch(req).then(function(odg){
      if(odg && odg.ok){ var kopija = odg.clone(); caches.open(KES).then(function(c){ c.put(req, kopija); }); }
      return odg;
    });
  }));
});
